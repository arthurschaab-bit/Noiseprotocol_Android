...
| **M12 Absturzsichere Diagnose** (ACRA fängt unbehandelte Abstürze ab, `BreadcrumbRingFile` übersteht den Prozesstod, vollständige `ApplicationExitInfo`-Auswertung inkl. ANR-Trace/natives Tombstone, streamender Support-Bundle-Export mit Größenbudget, automatischer Google-Drive-Upload für Absturz-/ANR-Bundles, tägliches Gesundheits-Bundle mit Kennzahlen, DiagnoseScreen-Absturzfix durch begrenzte Log-Anzeige) | ✅ umgesetzt, breit unit-/Compose-getestet — **nicht auf echter Hardware/Emulator geprüft** (kein Gerät in der Entwicklungsumgebung): ob der ursprünglich gemeldete Absturz behoben ist und ob Bundles tatsächlich in Drive ankommen, ist unbestätigter Verdacht, siehe [`docs/DIAGNOSE_CRASH_KONZEPT.md`](docs/DIAGNOSE_CRASH_KONZEPT.md) und `CHECKLISTE_GERAETETEST.md` Teil F |
