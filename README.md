...
| **Diagnose & Observability** (Sentry, DiagnosticsReporter, Redactor, Support-Paket) | ✅ abgeschlossen — Absturzsicherung seit M12 über ACRA, siehe Zeile unten |
...
| **Bugfixes 12.09.2026** (Huawei/EMUI-Button „Geschützte Apps prüfen" crashte, Aufnahme-Ausfall nach EBADF-Schreibfehler ohne automatischen Wiederanlauf, „+ Neue Messung" während laufender Messung sichtbar, Support-Bundle-Dateiname zu lang/falscher Dateityp, fehlgeschlagene WAV-Drive-Uploads unsichtbar im Diagnoseprotokoll) | ✅ behoben, unit-getestet — Huawei-Fix nur per simulierter `SecurityException` verifiziert, nicht erneut auf echtem Huawei-Gerät |
| **Protokollreiter nach Tagen gruppiert & erweitert filterbar** (dB-Bereich, Favoriten, Ruhezeiten, Messgerät/Mikrofon, kalibriert, Geräuschtyp — Sessions statt Einzelereignisse) | ✅ umgesetzt, Filterlogik unit-getestet — Optik/Bedienung des neuen Panels nicht am Gerät gesichtet |
| **PCE-323-Mikrofon-Fallback erkennbar gemacht** (Live-Cockpit zeigt „dB (Mikrofon-Fallback)" + Hinweistext statt eines unmarkierten Werts bei BLE-Verbindungsunterbrechung; Zeitraum-/Gesamtbericht-Diagramm mischt keine kalibrierten und unkalibrierten Werte mehr im selben Graphen) | ✅ umgesetzt, unit-getestet — Verhalten am echten PCE-323 während eines realen Verbindungsabbruchs nicht gesichtet |
| **M12 Absturzsichere Diagnose** (ACRA fängt unbehandelte Abstürze ab, `BreadcrumbRingFile` übersteht den Prozesstod, vollständige `ApplicationExitInfo`-Auswertung inkl. ANR-Trace/natives Tombstone, streamender Support-Bundle-Export mit Größenbudget, automatischer Google-Drive-Upload für Absturz-/ANR-Bundles, tägliches Gesundheits-Bundle mit Kennzahlen, DiagnoseScreen-Absturzfix durch begrenzte Log-Anzeige) | ✅ umgesetzt, breit unit-/Compose-getestet, instrumentierte Absturztests auf dem CI-Emulator (API 34) grün — **nicht auf echter Hardware geprüft**. ANRs erfasst ein eigener ANR-Watchdog auf allen Versionen (O-8); native Abstürze werden ab Android 11 gemeldet (mit Tombstone ab Android 12), auf Android 10 (Owner-Gerät Huawei P30) gar nicht (offener Punkt O-8, nativer Teil). Ob der ursprünglich gemeldete Absturz behoben ist und ob Bundles tatsächlich in Drive ankommen, ist unbestätigter Verdacht, siehe [`docs/DIAGNOSE_CRASH_KONZEPT.md`](docs/DIAGNOSE_CRASH_KONZEPT.md) und `CHECKLISTE_GERAETETEST.md` Teil F |
...
| [`docs/DIAGNOSE_OBSERVABILITY_KONZEPT.md`](docs/DIAGNOSE_OBSERVABILITY_KONZEPT.md) | **Diagnose-, Fehleranalyse- und Observability-Konzept** — Architektur für Sentry, Breadcrumbs, Redaction & Support-Bundle (umgesetzt; Statusangaben zu Sentry/Support-Bundle durch M12 korrigiert) |
| [`docs/DIAGNOSE_CRASH_KONZEPT.md`](docs/DIAGNOSE_CRASH_KONZEPT.md) | **M12: Absturzsichere Diagnose mit automatischem Drive-Upload (umgesetzt)** — ACRA-Absturzabfangen, absturzfeste Breadcrumbs, vollständige `ApplicationExitInfo`-Auswertung, streamender Support-Bundle-Export, automatischer Drive-Upload, tägliches Gesundheits-Bundle, DiagnoseScreen-Absturzfix; nicht auf Hardware geprüft |
| [`docs/PROMPT_M12_CRASH_DIAGNOSE.md`](docs/PROMPT_M12_CRASH_DIAGNOSE.md) | Umsetzungsauftrag für M12 in acht Schritten (erledigt) |
