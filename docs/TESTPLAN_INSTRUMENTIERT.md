...
| `NoiseRecordItem`: Abspielen-Icon / Karte selbst | `MainActivity.kt:702`, `734` | Tippen → `AudioPlayerScreen` öffnet mit der richtigen `filePath`. | Tippen auf eine Aufnahme mit nicht mehr existierender Datei → `AudioPlayerScreen` darf nicht mit einer nicht abfangenen `IOException` aus `MediaPlayer.prepare()` abstürzen (siehe Negativtest dort). |
...
| Play/Pause-Icon | `AudioPlayerScreen.kt:86` | Tippen → Wiedergabe startet, Fortschrittsbalken (`WaveformDisplay`) bewegt sich; erneutes Tippen pausiert. | **Bekannter Codemangel, gehört als Negativtest hierher:** `mediaPlayer.setDataSource(filePath)`/`.prepare()` in `DisposableEffect` (`AudioPlayerScreen.kt:32`) ist NICHT in einem `try`/`catch` - eine nicht mehr existierende oder beschädigte Datei (z. B. Aufnahme wurde parallel gelöscht, oder eine `.wav`-Datei mit defektem Header) lässt `prepare()` eine `IOException` werfen, die den Screen crasht. Dieser Negativtest deckt einen **echten, noch unbehobenen Bug** auf, keinen bereits gefixten - Datei vor dem Öffnen des Players im Hintergrund löschen und prüfen, dass die App nicht abstürzt (sollte künftig durch `try/catch` mit Fehlertext abgefangen werden, ähnlich `scanFehlermeldung()` in `MeterScreen.kt`). |
...
3. **Der `AudioPlayerScreen`-Absturz bei fehlender Datei** - kein Regressionstest für einen Fix,
   sondern ein Test, der einen bislang unentdeckten echten Bug **zuerst aufdeckt** (siehe Tabelle
   oben) - danach folgt der eigentliche Fix als separater Auftrag.
