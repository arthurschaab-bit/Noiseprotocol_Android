...
class GoogleDriveResumableUploadTest {
    ...
    @Test
    fun kleineDateiGehtInEinemBlockRausUndLiefertDieId() =
        runTest {
    ...
    @Test
    fun derSessionUriWirdVorDemErstenBlockGemeldet() =
        runTest {
    ...
    @Test
    fun dreihundertachtIstDerNormalfallZwischenZweiBloecken() =
        runTest {
    ...
    @Test
    fun derServerstandZaehlt_nichtDasWasGesendetSchien() =
        runTest {
    ...
    @Test
    fun abgebrochenerUploadNimmtAnDerGemeldetenPositionWiederAuf() =
        runTest {
            val groesse = BLOCK + 4096
    ...
    @Test
    fun einBereitsFertigerUploadWirdNichtNochEinmalGesendet() =
        runTest {
    ...
    @Test
    fun verfallenerSessionUriFuehrtZuGenauEinemNeuanfang() =
        runTest {
    ...
    @Test
    fun einZweitesVierhundertvierBrichtAbStattEndlosZuWiederholen() =
        runTest {
            server.enqueue(MockResponse().setResponseCode(404))
            server.enqueue(MockResponse().setResponseCode(200).setHeader("Location", sessionUri()))
            server.enqueue(MockResponse().setResponseCode(404))
    ...
    @Test
    fun einStehengebliebenerServerstandBrichtAbStattEwigZuLaufen() =
        runTest {
    ...
    @Test
    fun eineSitzungOhneLocationHeaderIstEinFehler() =
        runTest {
    ...
    @Test
    fun aktualisierenUeberMehrereBloeckeFunktioniertWieDieNeuanlage() =
        runTest {
    ...
    @Test
    fun aktualisierenNimmtEineUnterbrocheneUebertragungAmGemeldetenStandWiederAuf() =
        runTest {
            val groesse = BLOCK + 4096
            server.enqueue(MockResponse().setResponseCode(308).setHeader("Range", "bytes=0-${BLOCK - 1}"))
            ...
            val gestartet = mutableListOf<String>()
            val ergebnis =
                client.dateiAktualisierenResumable(
                    "bestehend-3",
                    datei(groesse),
                    "application/zip",
                    fortsetzenAb = sessionUri(),
    ...
    @Test
    fun derHochgeladeneInhaltIstByteGleichZurDatei() =
        runTest {
