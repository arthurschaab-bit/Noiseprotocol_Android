...
class DriveSyncCoordinatorTest {
    private class FakeLevelSampleDao : LevelSampleDao {
        ...
        override suspend fun zwischen(
            von: Long,
            bis: Long,
        ): List<LevelSampleEntity> = eingefuegt.filter { it.at in von until bis }
    ...
    private class FakeDailyFileDao : DriveDailyFileDao {
        ...
        override suspend fun upsert(entity: DriveDailyFileEntity) {
            zeilen[entity.date] = entity
        }

        override suspend fun update(entity: DriveDailyFileEntity) {
            zeilen[entity.date] = entity
        }
    ...
    private open class FakeNoiseDao : NoiseDao {
        ...
        override suspend fun softDeleteMultiple(
            ids: List<Long>,
            deletedAt: Long,
        ) {}
        ...
        override suspend fun setDetectedLabel(
            id: Long,
            label: String?,
        ) {}
    ...
    private class FakeDriveApiClient : DriveApiClient {
        ...
        override suspend fun ordnerAnlegen(
            name: String,
            elternId: String?,
        ) = ordnerAnlegenErgebnis

        override suspend fun ordnerSuchen(
            name: String,
            elternId: String?,
        ) = ordnerSuchenErgebnis
    ...
    private class FakeBeweisVideoDao(
        gespeichert: List<com.example.lrmprotokoll.data.BeweisVideoEntity> = emptyList(),
        ...
        override suspend fun setzeDriveFileId(
            id: Long,
            fileId: String,
        ) {
        ...
        override suspend fun setzeUploadFortschritt(
            id: Long,
            sessionUri: String?,
            bytes: Long,
        ) {
        ...
        override suspend fun setzeAufnahmeergebnis(
            id: Long,
            dauerMs: Long,
            groesseBytes: Long,
        ) {
        ...
        override suspend fun setzeGemuxt(
            id: Long,
            dateiPfad: String,
            groesseBytes: Long,
            hatTonspur: Boolean,
            ...
            zeilen[id] =
                zeilen.getValue(id).copy(
                    dateiPfad = dateiPfad,
                    groesseBytes = groesseBytes,
                    hatTonspur = hatTonspur,
                    tonGemuxt = true,
                    muxFehlgeschlagen = false,
                )
        }

        override suspend fun loesche(id: Long) {
            zeilen.remove(id)
        }
    }
    ...
    private fun fuegeSampleHinzu(
        sekundenSeitMitternacht: Long,
        db: Double,
    ) {
        val mitternacht =
            uhr
    ...
    private fun fuegeSampleFuerTagHinzu(
        tageZurueck: Long,
        sekundenSeitMitternacht: Long,
        db: Double,
    ) {
    ...
    @Test
    fun einVersaeumterGestrigerTagWirdNachgeholt() =
        runTest {
    ...
    @Test
    fun einBereitsVollstaendigSynchronisierterVersaeumterTagWirdNichtErneutHochgeladen() =
        runTest {
            fuegeSampleFuerTagHinzu(tageZurueck = 1, sekundenSeitMitternacht = 3600, db = 55.0)
            val gestern =
                uhr
            ...
            assertEquals(
                ...
                0,
                driveApi.anlegenAufrufe,
            )
            assertEquals(0, driveApi.aktualisierenAufrufe)
        }

    @Test
    fun einFehlschlagBeiEinemVersaeumtenTagVerhindertNichtDenSyncFuerHeute() =
        runTest {
            fuegeSampleFuerTagHinzu(tageZurueck = 1, sekundenSeitMitternacht = 3600, db = 55.0)
    ...
    @Test
    fun einSampleAelterAls30TageWirdBeimSyncGeloescht() =
        runTest {
    ...
    @Test
    fun einSampleInnerhalbVon30TagenUeberlebtDenSync() =
        runTest {
    ...
    @Test
    fun ohneEingerichtetenOrdnerWirdNichtsHochgeladen() =
        runTest {
    ...
    @Test
    fun blockierterOrdnerWirdNichtErneutVersucht() =
        runTest {
    ...
    @Test
    fun ersterZyklusLegtEineNeueDateiAn() =
        runTest {
    ...
    @Test
    fun zweiterZyklusMitBekannterFileIdAktualisiertStattNeuAnzulegen() =
        runTest {
            fuegeSampleHinzu(0, 55.0)
            baueKoordinator().syncEinenZyklus()
     ...
     */
    @Test
    fun doppelterZyklusZurSelbenZeitUeberspringtDenErneutenUpload() =
        runTest {
            fuegeSampleHinzu(0, 55.0)
    ...
    @Test
    fun verstricheneZeitMitNeuemSampleErzeugtLueckenzeilenUndWirdHochgeladen() =
        runTest {
            fuegeSampleHinzu(0, 55.0)
            baueKoordinator().syncEinenZyklus()
     ...
     */
    @Test
    fun syncFunktioniertAuchWennDasEchteKalenderdatumWeitVonDerTestuhrAbweicht() =
        runTest {
    ...
    @Test
    fun waiseAusVorherigemAbgebrochenemZyklusWirdGefundenStattDoppeltAngelegt() =
        runTest {
    ...
    @Test
    fun vierNullVierBeiBekannterFileIdVerwirftNurDieFileIdNichtDenOrdner() =
        runTest {
            fuegeSampleHinzu(0, 55.0)
    ...
    @Test
    fun vierNullVierOhneBekannteFileIdBlockiertDenOrdner() =
        runTest {
    ...
    @Test
    fun netzwerkfehlerWirdAlsFehlschlagOhneOrdnerBlockadeVermerkt() =
        runTest {
    ...
    @Test
    fun erfolgSetztDenFehlschlagszaehlerZurueck() =
        runTest {
    ...
    @Test
    fun aggregationsintervallAusDenEinstellungenWirdVerwendet() =
        runTest {
    ...
    @Test
    fun gzipWirdBeimUploadVerwendet() =
        runTest {
            var gzipGesehen = false
            val pruefenderClient =
                object : DriveApiClient by driveApi {
                    override suspend fun dateiAnlegen(
                        name: String,
                        ordnerId: String,
                        inhalt: ByteArray,
    ...
    @Test
    fun wavUploadWirdErfolgreichSynchronisiertAuchWennKeinePegelzeilenVorliegen() =
        runTest {
            val tempWav =
                java.io.File.createTempFile("test_audio", ".wav").apply {
                    writeBytes(ByteArray(100) { 1 })
                    deleteOnExit()
                }
            val customNoiseDao =
                object : FakeNoiseDao() {
                    override suspend fun getAlleAktiven(): List<NoiseRecord> =
                        listOf(
                            NoiseRecord(
                                id = 1L,
     ...
     */
    @Test
    fun fehlgeschlagenerWavUploadWirdImDiagnoseprotokollGemeldet() =
        runTest {
            val tempWav =
                java.io.File.createTempFile("test_audio", ".wav").apply {
                    writeBytes(ByteArray(100) { 1 })
            ...
            val reporter =
                com.example.lrmprotokoll.diagnose.CompositeDiagnosticsReporter(
                    initialContext =
                        com.example.lrmprotokoll.diagnose
                            .DiagnosticContext(appVersion = "1.0", buildType = "debug"),
                )
    ...
    private fun videoEintrag(
        ...
        sessionId = 5,
        dateiPfad = pfad,
        gestartetAm = uhr.now().toEpochMilli(),
        dauerMs = 30_000,
        hatTonspur = true,
        groesseBytes = 4096,
        tonGemuxt = tonGemuxt,
    ...
    private fun tempVideo() =
        java.io.File.createTempFile("beweisvideo", ".mp4").apply {
    ...
    private fun koordinatorMitVideos(
        dao: FakeBeweisVideoDao,
    ...
    @Test
    fun mitZustimmungWirdUeberDenResumablePfadHochgeladen() =
        runTest {
    ...
    @Test
    fun einNochNichtGemuxtesVideoBleibtLiegen() =
        runTest {
    ...
    @Test
    fun einBereitsHochgeladenesVideoWirdNichtNochEinmalGesendet() =
        runTest {
            settings.videoDriveUpload = true
    ...
    @Test
    fun einAngefangenerUploadWirdFortgesetztStattNeuBegonnen() =
        runTest {
            settings.videoDriveUpload = true
            val dao =
                FakeBeweisVideoDao(
    ...
    @Test
    fun derFortschrittWirdGespeichertDamitEinAbbruchFortsetzbarBleibt() =
        runTest {
    ...
    @Test
    fun einVideoOhneDateiWirdUebersprungenStattDenZyklusZuSprengen() =
        runTest {
            settings.videoDriveUpload = true
    ...
    @Test
    fun ohneQuelleGehtKeineDatenbankSicherungRaus() =
        runTest {
    ...
    @Test
    fun beiDeaktiviertemSchalterGehtKeineDatenbankSicherungRausObwohlEineQuelleDaIst() =
        runTest {
            settings.datenbankSicherungDriveUpload = false
            var quelleAufgerufen = false

            koordinatorMitDatenbankSicherung({
                quelleAufgerufen = true
                tempSicherungsDatei()
            }).syncEinenZyklus()
    ...
    @Test
    fun mitAktiviertemSchalterWirdDieDatenbankSicherungHochgeladen() =
        runTest {
    ...
    @Test
    fun eineBestehendeSicherungWirdAktualisiertStattEinerZweiten() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
    ...
    @Test
    fun einFehlerBeimErstellenDerSicherungReisstDenRestDesZyklusNichtMit() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
     ...
     */
    @Test
    fun platzmangelBeimBauenMeldetBackupCreateFailedUndLaedtNichtsHoch() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
            val diagnoseContext =
                com.example.lrmprotokoll.diagnose
     ...
     */
    @Test
    fun erfolgreicherSicherungsUploadSetztEigenenZeitstempel() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
    ...
    @Test
    fun fehlgeschlagenerSicherungsUploadSetztDenZeitstempelNicht() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
     ...
     */
    @Test
    fun zweiterVersuchKurzNachDemErstenWirdUebersprungen() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
            var quelleAufrufe = 0
     ...
     */
    @Test
    fun zweiterVersuchNachFehlgeschlagenemErstenVersuchWirdEbenfallsUebersprungen() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
            ...
            koordinatorMitDatenbankSicherung { throw java.io.IOException("Checkpoint fehlgeschlagen") }
                .syncEinenZyklus()
            koordinatorMitDatenbankSicherung {
                zweiteQuelleAufgerufen = true
                tempSicherungsDatei()
            }.syncEinenZyklus()

            assertTrue(
                "Nach einem fehlgeschlagenen Versuch darf der naechste innerhalb des Intervalls nicht erneut versuchen",
                !zweiteQuelleAufgerufen,
            )
        }

    @Test
    fun nachAblaufDesIntervallsWirdErneutVersucht() =
        runTest {
            settings.datenbankSicherungDriveUpload = true
            var quelleAufrufe = 0
     ...
     *
     * Seit der Nachbesserung vom 24.09.2026 OHNE Ausnahme fuer eine Fensterdauer, die 60s nicht
     * glatt teilt (`PegelAggregator.aggregiere()` gruppiert Rohwerte jetzt auf dem absoluten
     * Millisekunden-Raster statt relativ zu `von`, siehe dessen KDoc) - der frueher hier
     * dokumentierte Fallback auf einen Gesamtaufruf bei nicht rasteraligniertem `von` ist
     * entfallen, deshalb jetzt inklusive 7s in der Liste unten. Die Gegenprobe dazu (vormals
     * `beiNichtTeilenderFensterdauerFaelltDieAggregationAufEinenGesamtaufrufZurueck`) ist mit dem
     * Fallback selbst entfallen.
     */
    @Test
     ...
     * Werte). Geprueft fuer mehrere Fensterdauern, darunter eine (7s), die 3600 nicht teilt: Vor
     * der Nachbesserung vom 24.09.2026 verglich dieser Fall trivial ein und denselben Fallback-
     * Codepfad mit sich selbst (siehe `aggregiereInAbschnitten()`-KDoc); seit dessen Entfernung
     * prueft er hier erstmals wirklich, dass echte stueckweise Aggregation (mit Abschnitts-
     * Stitching) fuer nicht rasteraligniertes `von` dasselbe liefert wie ein Gesamtaufruf.
     */
    @Test
     ...
     * Luecke zu pruefen, die einen Abschnitt nur teilweise fuellt. Wie in Test 3 ist 7s seit der
     * Nachbesserung vom 24.09.2026 kein Fallback-Sonderfall mehr, sondern echte stueckweise
     * Aggregation mit Abschnitts-Stitching.
     */
    @Test
